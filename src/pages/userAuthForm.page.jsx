...
const UserAuthForm = ({ type }) => {
    ...
    const authForm = useRef(); // Ensure it's initialized to null


    let { userAuth: { accessToken, refreshToken, data }, setUserAuth } = useContext(UserContext);
    ...
    const userAuthThroughServer = (serverRoute, formData) => {
        const baseURL = process.env.REACT_APP_URL || "http://localhost:8000/api/v1"; // Fallback URL
        axios
            .post(baseURL + "/user" + serverRoute, formData)
            // .post("http://localhost:8000/api/v1" + serverRoute, formData)
            .then(({ data }) => {
    ...
    const handleGoogleAuth = (e) => {
        ...
        authWithGoogle()
            .then(user => {
                console.log("Google User:", user); // Log the user returned from Google Auth
                let serverRoute = "/google-auth";
