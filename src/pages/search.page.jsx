...
const SearchPage = () => {
    ...
    const searchBlogs = async ({ page = 1, create_new_arr = false }) => {
        try {
            const response = await axios.post(process.env.REACT_APP_URL + "/blog/search-blogs", { query, page })
    ...
    const fetchUsers = async () => {
        await axios.post(process.env.REACT_APP_URL + "/user/search-users", { query })
            .then(({ data }) => {
    ...
    return (
        ...
        <section className="h-cover flex justify-center gap-10">
            ...
            <div className="w-full">
                {/* <InPageNavigation routes={[`Search Results from "${query}"`, `Accounts Matched`]} defaultHidden={['Account Matched']}> */}
